 ...
 */
internal class PaylisherQueue(
    private val config: PaylisherConfig,
    private val api: PaylisherApi,
    private val endpoint: PaylisherApiEndpoint,
    ...
    private fun batchEvents() {
        ...
        for (file in files) {
            try {
                ...
                inputStream.use {
                    val event = config.serializer.deserialize<PaylisherEvent?>(it.reader().buffered())
                    event?.let { theEvent ->
                        events.add(theEvent)
                    }
                }
